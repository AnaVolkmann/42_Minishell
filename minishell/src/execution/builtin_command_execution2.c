 ...
 */
int	execute_builtin_with_piping(char **_cmd_, int *_fd, t_env *env, int *pipe_data)
{
	int				status;
	int				_out_fd[2];
...
int	execute_builtin_with_simple_piping(
		char **_cmd_, int *_fd, t_env *env, int *pipe_data)
{
	int				status;
	int				_out_fd[2];
...
int	manage_single_builtin_execution(
		char **cmd, int *_fd, t_env *env, int *pipe_data)
{
	int				status;
	int				ex_status;
	...
	if (str_cmp(cmd[0], "exit", NULL))
	{
		...
		if (cmd[1] && !is_string_numeric(cmd[1]))
			...
			ex_status = string_to_int(cmd[1]);
		free_string_array(cmd);
		ft_putendl_fd("EXIT", 1);
		cleanup_and_exit_shell(env, ex_status);
	}
	else if (!pipe_data[8])
		status = execute_builtin_with_piping(cmd, _fd, env, pipe_data);
	else
		status = execute_builtin_with_simple_piping(cmd, _fd, env, pipe_data);
	return (status);
}
