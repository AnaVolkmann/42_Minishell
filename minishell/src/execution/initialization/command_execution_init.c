...
/*   Updated: 2025/03/21 17:13:37 by alawrence        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * This function checks the given path and file for potential
 * directory errors. It ensures that the path does not point 
 * to an invalid directory or a file when the file is expected 
 * to be something else. It also handles specific error messages 
 * for common issues like attempting to work with directories when
 *  files are expected.
 * @param path_ The path of the file or directory to check.
 * @param file The name of the file or directory to verify.
 * @param status A pointer to the status variable which is 
 * updated based on the check.
 */
 ...
 *This function processes and prints error messages for file
 *handling. It displays appropriate error messages based on the
 *status code
 *provided, helping to identify issues with files and directories.
 * @param file The name of the file associated with the error.
 * @param _status The status code indicating the type of error 
 * encountered.
...
int	handle_file_error_message(char *file, int _status)
{
	...
	else if (_status)
	{
		...
		ft_putstr_fd(file, 2);
		ft_putendl_fd(
			"': Operation failed, check input or environment.", 2);
	}
	return (_status);
}

/** @brief cheks the file permissions for a command.
 * This function checks if the file associated with a command 
 * has the appropriate read or write permissions based on its type.
 *  It handles error checking
 *  and ensures that files can be accessed or processed as expected.
 ...
 * @return The status code indicating whether the file
 * permissions are valid or not.*/
 ...
 * This function iterates over an Abstract Syntax Tree (AST) and
 *  adjusts the file type of each node based on the redirection or 
 * piping that is specified. It sets flags for redirections, pipes,
 *  and file types for commands to be executed.
 ...
 * This function counts how many redirection and pipe operations 
 * are present in the AST. It updates an array of counters that track 
 * the number of output redirections, input redirections, pipes, and 
 * other relevant details for the command execution.
