...
/*   Updated: 2025/03/16 21:13:25 by ana-lda-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * 
 * Validates if the character at the specified index in the string is the start of a valid variable, based on whether the condition (`con`) is met.
 * If `con` is true, checks for variables that start with a dollar sign `$` followed by valid characters. 
 * If `con` is false, checks for variables that do not start with `$` and consist of alphanumeric characters or `_` or `?`.
 * 
...
/** 
 * @brief Appends a new value to an existing environment variable or creates a new one.
 * 
 * If the environment variable already exists, the function appends the new value to it (handling the case where `+` is present in the input).
 * If the variable does not exist, it creates a new entry with the given value.
 * 
 ...
 * 
 * Searches for a matching environment variable name and returns its index in the parsed environment.
